...
var BoardData = function (name) {
  ...
  this.cpFile = path.join(
    config.HISTORY_DIR,
    "board-" + encodeURIComponent(name) + "_cp_bu.json"
  );
...
BoardData.prototype.saveCP = function (action) {
  switch (action) {
    case "saveBoard":
      this.boardCheckPoints.length = ++this.currCP;
      let currBoard = JSON.parse(JSON.stringify(this.board));
      this.boardCheckPoints.push(currBoard);

      if (5 < this.boardCheckPoints.length) {
        this.boardCheckPoints.shift();
        --this.currCP;
      }
    ...
    case "back":
      if (0 <= this.currCP) {
        this.board = JSON.parse(JSON.stringify(this.boardCheckPoints[this.currCP--]));
      }
      
      break;

    case "next":
      if (this.boardCheckPoints.length > this.currCP + 1) {
        this.board = JSON.parse(JSON.stringify(this.boardCheckPoints[++this.currCP]));
      }
...
BoardData.prototype.save = async function (file) {
  ...
  var cp_tmp_file = backupFileName(cpFile);
  var cp_bu_txt = JSON.stringify({arr: this.boardCheckPoints, index: this.currCP});

  try {
    await fs.promises.writeFile(cp_tmp_file, cp_bu_txt);
    await fs.promises.rename(cp_tmp_file, cpFile);
  } catch (err) {
    log("board cp saving error", {
      err: err.toString(),
      tmp_file: tmp_file,
    });
    return;
...
BoardData.load = async function loadBoard(name) {
  ...
  try {
    boardCP = await fs.promises.readFile(boardData.cpFile);
    let cpData = JSON.parse(boardCP);
    boardData.boardCheckPoints = cpData.arr;
    boardData.currCP = cpData.index;
  } catch (e) {
    boardData.boardCheckPoints = [];
  }
